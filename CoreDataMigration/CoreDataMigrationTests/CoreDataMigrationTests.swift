...
let MyBundle = Bundle(for: PlainCoreDataMigrationTests.self)
...
class PlainCoreDataMigrationTests: XCTestCase {
    ...
    func testMigraion_v1_0_0() throws {
        ...
        XCTAssertEqual(results[0].value(forKey: "v2_0_0_name") as? String, "A Known Value")
    }
    
    func testMigration_v1_1_0() throws {
        try FileManager.default.moveItem(at: Example_v1_1_0, to: self.exampleStoreDestination)
        let stack = try Stack(self.containerType)
        let results = try stack.fetchAll()
        XCTAssertEqual(results.count, 1)
        XCTAssertEqual(results[0].value(forKey: "v2_0_0_name") as? String, "A Known Value")
    ...
    func testMigration_v1_1_2() throws {
        try FileManager.default.moveItem(at: Example_v1_1_2, to: self.exampleStoreDestination)
        let stack = try Stack(self.containerType)
        let results = try stack.fetchAll()
        XCTAssertEqual(results.count, 1)
        XCTAssertEqual(results[0].value(forKey: "v2_0_0_name") as? String, "A Known Value")
    ...
    func testMigration_v1_2_0() throws {
        try FileManager.default.moveItem(at: Example_v1_2_0, to: self.exampleStoreDestination)
        let stack = try Stack(self.containerType)
        let results = try stack.fetchAll()
        XCTAssertEqual(results.count, 1)
        XCTAssertEqual(results[0].value(forKey: "v2_0_0_name") as? String, "A Known Value")
    ...
    func testMigration_v2_0_0() throws {
        try FileManager.default.moveItem(at: Example_v2_0_0, to: self.exampleStoreDestination)
        let stack = try Stack(self.containerType)
        let results = try stack.fetchAll()
        XCTAssertEqual(results.count, 1)
        XCTAssertEqual(results[0].value(forKey: "v2_0_0_name") as? String, "A Known Value")
